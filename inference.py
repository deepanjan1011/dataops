...
DATAOPS_GYM_URL = os.getenv("DATAOPS_GYM_URL", "http://localhost:7860")
BENCHMARK = os.getenv("DATAOPS_BENCHMARK", "dataops_gym")


# ---------------------------------------------------------------------------
# Structured logging — exact format required by evaluation harness
# ---------------------------------------------------------------------------
...
PHASE 2 — Clean the purchases table:
...
def run_task(task_id: str) -> float:
    """Run the agent on a single task. Returns grader score."""
    ...
    try:
        for step in range(1, max_steps + 1):
            ...
            if error:
                user_content = (
                    f"YOUR LAST ACTION FAILED WITH ERROR: {error}\n"
            ...
            # Step environment
            try:
                step_resp = requests.post(f"{DATAOPS_GYM_URL}/step", json=action)
                step_resp.raise_for_status()
                obs = step_resp.json()
            except Exception as e:
                rewards.append(0.0)
                step_count = step
                log_step(step=step, action=action_str, reward=0.00, done=False, error=str(e))
                break

            done = obs.get("done", False)
            step_error = obs.get("error")
            # Use health score delta as reward signal
            reward = obs.get("data_health_score", 0.0)
        ...
        # 3. Get grader score
        try:
            grade_resp = requests.post(f"{DATAOPS_GYM_URL}/grader")
...
ALL_TASKS = [
    "easy", "medium", "hard", "outlier_detection",
    "schema_migration", "drift_detection", "poisoning_detection",
]


def run_all_tasks() -> dict:
    """Run agent on all 7 tasks and return scores."""
    if not HF_TOKEN:
